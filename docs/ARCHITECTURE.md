...
**The Hack:** Before starting any gallery queue, the extension synchronously downloads a dummy text file (`.md-keep`) into the target subfolder, waits for the native file write to finish so the directory is _guaranteed_ to exist, and then silently deletes the dummy file. Chromium sees the directory exists, skips the buggy `mkdir` race entirely, and your browser survives.
...
**The Hack:** The Service Worker performs an explicit `HEAD` (or `GET`) request before passing the URL to the download manager. If the `Content-Type` is `text/html`, we throw an error and abort the download _before_ Chrome can touch it, preventing HTML litter in your downloads folder.
...
**The Bug:** If the CDN responds _too quickly_ (e.g., edge cached or tiny image), the network header arrives and `onDeterminingFilename` fires **before** the `downloads.download()` promise even resolves. Because your mapping hasn't been saved yet, the listener throws up its hands, Chrome uses the CDN's garbage filename, strips your subfolder, and drops the file directly into `~/Downloads`.
...
**The Scoped Fix:** A permanently registered `onDeterminingFilename` listener changes filename resolution for every browser download, including downloads started by other extensions such as Video DownloadHelper. The listener is therefore installed immediately before one of our own `downloads.download()` calls and removed as soon as all pre-registered Clanker filenames have been consumed. Registrations are keyed by URL so the listener can supply the intended filename even when the event fires before `downloads.download()` resolves with its ID. A safety timer removes an abandoned registration if Chrome never emits the event.
